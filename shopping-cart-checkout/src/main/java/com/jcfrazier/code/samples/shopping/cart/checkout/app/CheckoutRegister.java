...
import com.jcfrazier.code.samples.shopping.cart.checkout.system.Commodity;
import com.jcfrazier.code.samples.shopping.cart.checkout.system.CommodityInventory;
import com.jcfrazier.code.samples.shopping.cart.checkout.system.CommodityInventoryUtil;
import com.jcfrazier.code.samples.shopping.cart.checkout.system.RegularCommodityCheckout;
import com.jcfrazier.code.samples.shopping.cart.checkout.system.SimpleOffer;
...
public class CheckoutRegister {
	...
	private static final List<SimpleOffer> SIMPLE_OFFERS = createSimpleOffers();

	public static void main(String[] args) {
		final RegularCommodityCheckout regularCheckout = new RegularCommodityCheckout(COMMODITY_INVENTORY);
		final SimpleOfferCommodityCheckout simpleOfferCheckout = new SimpleOfferCommodityCheckout(
				SIMPLE_OFFERS, regularCheckout);
		...
		while (!(input = scanner.next()).equals(EXIT_PROMPT_FLAG)) {
			...
			case INPUT_COMPLETE_FLAG:
				printCheckoutOuput(shoppingCart, simpleOfferCheckout);
				break;
			default:
	...
	private static final List<SimpleOffer> createSimpleOffers() {
		return CommodityInventoryUtil.createCommodities().stream().map(CheckoutRegister::createSimpleOfferForCommodity)
				.collect(Collectors.toList());
	}

	private static final SimpleOffer createSimpleOfferForCommodity(Commodity item) {
		String commodityId = item.getId();
		if (commodityId.equals("0")) {
			return new SimpleOffer(item, 2);
		} else if (commodityId.equals("1")) {
			return new SimpleOffer(item, 3);
		} else {
			return new SimpleOffer(item, 1);
		}
	}
}
